...
class Lang:
    def __init__(self, config_filename="lang_config.json", guilds_langs_filename="guilds_langs.json"):
        self.config_filename = config_filename
        self.guilds_langs_filename = guilds_langs_filename

    def set_lang(self, guild_id: int, code: str) -> bool:
        """Sets guild's lang

        Args:
            guild_id (int): Guild id
            code (str): Code of lang

        Returns:
            bool: Set or not. Maybe wrong code.
        """
        with open(self.config_filename,'r', encoding='UTF-8') as file:
            config = json.loads(file.read())
        if code not in config["codes"]: return False
        with open(self.guilds_langs_filename, 'r', encoding='UTF-8') as file:
            guilds: dict = json.loads(file.read())
        guilds.update({str(guild_id): code})
        with open(self.guilds_langs_filename, 'w', encoding='UTF-8') as file:
...
async def lang_command(ctx: Context, *args):
    """Sets bot's language by lang_code

    Args:
        lang_code (str): Language code. Could be shown by `lang list`
    """
    if len(args) == 1:
        if args[0] == 'codes':
            codes_str = ""
