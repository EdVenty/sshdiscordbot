...
async def start(ctx: Context, ip, user, password):
    ...
    try:
        ...
        log.info("Сбой подключения к {}. Неизвестная ошибка. Трэйсбэк код: {}.".format(ip, err_code))
        raise err


@bot.command(pass_context=True)
async def traceback(ctx: Context, code):
    """Получить трэйсбэк по коду"""
...
async def send(ctx: Context, *, command):
    """Выполнить команду на текущем ssh подключении"""
...
async def disconnect(ctx: Context):
    """Прекратить общение с ssh соединением"""
    if channel_connections.__contains__(ctx.channel.id):
...
async def clist(ctx: Context):
    """Получить список соединений для вас"""
    user_id = ctx.author.id
    user_connections = []
    for connection in sshs.sshs:
        if sshs.sshs[connection]["user_id"] == user_id:
            user_connections.append({"id": connection, "connection_name": sshs.sshs[connection]["connection_name"]})
    if user_connections == []:
        ...
        if channel_connections.__contains__(ctx.channel.id):
            if channel_connections[ctx.channel.id] == dict_conn['id']:
                ...
                if ctx.channel.type != discord.ChannelType.private:
                    out += lang["bot.command.clist.message.ssh_connection_now_channel"].format(i, dict_conn["connection_name"], dict_conn["id"], ctx.channel.name)
                else:
...
async def hamachi(ctx: Context, *args):  
    if len(args) == 0:
        ...
        if args[0] == "join":
            ...
            cmd = ["sudo hamachi join {} {}".format(hamachi_id, hamachi_password)]
            result = subprocess.run("sudo hamachi join {} {}".format(hamachi_id, hamachi_password), shell=True, stderr=subprocess.PIPE, stdout=subprocess.PIPE, encoding='CP866')
...
async def help(ctx: Context, *args): 
    if len(args) == 0:
        embed=discord.Embed(description=lang["bot.embed.description"], color=0xe88617)
        embed.set_thumbnail(url="https://cdn.discordapp.com/app-icons/752522095296118845/236e26f1a371364e408b7621d04df5e0.png?size=128")
        embed.set_author(name=lang["bot.embed.author.name"], icon_url="https://cdn.discordapp.com/app-icons/752522095296118845/236e26f1a371364e408b7621d04df5e0.png?size=64")
        embed.add_field(name=lang["bot.embed.field.commands.name"], value=lang["bot.embed.field.commands.command"])
        embed.add_field(name=lang["bot.embed.field.docs.name"], value=lang["bot.embed.field.docs.command"], inline=True)
        embed.add_field(name=lang["bot.embed.admin.name"], value=lang["bot.embed.admin.link"])
        await ctx.send(embed=embed)
    else:
        if args[0] == "commands":
            embed = discord.Embed(title=lang["bot.embed.args.command.title"], color=0xe88617)
            embed.add_field(name=lang["bot.command.start.name"], value=lang["bot.command.start.description"], inline=False)
            embed.add_field(name=lang["bot.command.traceback.name"], value=lang["bot.command.traceback.description"], inline=False)
            embed.add_field(name=lang["bot.command.connect.name"], value=lang["bot.command.connect.description"], inline=False)
            embed.add_field(name=lang["bot.command.send.name"], value=lang["bot.command.send.description"], inline=False)
            embed.add_field(name=lang["bot.command.end.name"], value=lang["bot.command.end.description"], inline=False)
            embed.add_field(name=lang["bot.command.disconnect.name"], value=lang["bot.command.disconnect.description"], inline=False)
            embed.add_field(name=lang["bot.command.clist.name"], value=lang["bot.command.clist.description"], inline=False)
            embed.add_field(name=lang["bot.command.hamachi.name"], value=lang["bot.command.hamachi.description"], inline=False)
            embed.set_footer(text=lang["bot.embed.footer.commands.text"])
            await ctx.send(embed=embed)
        elif args[0] == "docs":
            embed = discord.Embed(title=lang["bot.embed.docs.field.title"], description=lang["bot.embed.docs.field.description"], color=0xe88617)
            embed.add_field(name=lang["bot.embed.docs.field.start.name"], value=lang["bot.embed.docs.field.start.description"], inline=False)
            embed.add_field(name=lang["bot.embed.docs.field.main.start"], value=lang["bot.embed.docs.field.main.description"])
            ...
            if sshs.__contains__(channel_connections.connections[discord_channel]):
                ...
                if channel.recv_stderr_ready():
                    try:
                        ...
                        pass
                if data != '':
                    messages_to_send.append({"channel_id": discord_channel, "text": data})
